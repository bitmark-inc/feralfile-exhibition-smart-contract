const FeralfileExhibitionV4_3 = artifacts.require("FeralfileExhibitionV4_3");
...
contract("FeralfileExhibitionV4_3", async (accounts) => {
    before(async function () {
        this.signer = accounts[0];
        this.vault = await FeralfileVault.new(this.signer);
        this.seriesIds = [0, 1, 2, 3, 4];
        this.seriesMaxSupply = [1, 10, 100, 1000, 10000];
        this.mergeArtworkInfo = [1, 2, 2000000];

        // Deploy multiple contracts
        this.contracts = [];
        for (let i = 0; i < 13; i++) {
            let contract = await FeralfileExhibitionV4_3.new(
                "Feral File V4 Test",
                "FFv4",
                true,
                true,
    ...
    it("test duplicate series in constructor", async function () {
        ...
        let mergeArtworkInfo = [1, 2, 2000000];
        try {
            await FeralfileExhibitionV4_3.new(
                "Feral File V4 Test",
                "FFv4",
                true,
                true,
    ...
    it("test advance amount", async function () {
        ...
        const mergeArtworkInfo = [1, 2, 2000000];
        const advanceAddresses = [accounts[3], accounts[4]];
        const advanceAmounts = [
            web3.utils.toWei("0.3", "ether"),
            web3.utils.toWei("0.8", "ether"),
        ];
        const contract = await FeralfileExhibitionV4_3.new(
            "Feral File V4 Test",
            "FFv4",
            true,
            true,
        ...
        const tokenID2 = seriesIds[0] * 1000000 + 1;
        const tokenID3 = seriesIds[1] * 1000000 + 0;
        const tokenID4 = seriesIds[1] * 1000000 + 1;
        const data = [
            [seriesIds[0], tokenID1, contract.address],
            [seriesIds[0], tokenID2, contract.address],
            [seriesIds[1], tokenID3, contract.address],
            [seriesIds[1], tokenID4, contract.address],
        ];
        await contract.mintArtworks(data);
    ...
    it("test merge artworks successfully", async function () {
        const contract = this.contracts[11];

        // 1. Prepare data
        const owner = accounts[1];
        const tokenId1 = this.seriesIds[1] * 1000000 + 0;
        const tokenId2 = this.seriesIds[1] * 1000000 + 1;
        const tokenId3 = this.seriesIds[1] * 1000000 + 2;
        let mintData = [
            [this.seriesIds[1], tokenId1, owner],
            [this.seriesIds[1], tokenId2, owner],
            [this.seriesIds[1], tokenId3, owner],
        ];
        let tx = await contract.mintArtworks(mintData);
    ...
    it("test merge artworks failed invalid owner", async function () {
        ...
        let mintData = [
            [this.seriesIds[1], tokenId1, owner],
            [this.seriesIds[1], tokenId2, accounts[2]],
            [this.seriesIds[1], tokenId3, owner],
        ];
        let tx = await contract.mintArtworks(mintData);
    ...
    it("test merge artworks failed invalid seriesID", async function () {
        const contract = this.contracts[12];

        // 1. Prepare data
        const owner = accounts[1];
        const tokenId1 = this.seriesIds[1] * 1000000 + 0;
        const tokenId2 = this.seriesIds[1] * 1000000 + 1;
        const tokenId3 = this.seriesIds[3] * 1000000 + 2;
        let mintData = [
            [this.seriesIds[1], tokenId1, owner],
            [this.seriesIds[1], tokenId2, owner],
            [this.seriesIds[1], tokenId3, owner],
