...
contract("MoMABurn", async (accounts) => {
    ...
    it("test mint and burn not enabled", async function () {
        try {
            ...
            assert.include(
                error.message,
                "Burn and mint status is not enabled"
            );
        }
    });
    ...
    it("test mint and burn successfully for tier 1", async function () {
        ...
        const ownerNewEdition =
            await this.exhibitionTier1.ownerOf(newEditionID);

        const owner1New = await this.exhibitionV2.ownerOf(
            editionID1.toString()
        );
        const owner2New = await this.exhibitionV2.ownerOf(
            editionID2.toString()
        );
        const owner3New = await this.exhibitionV2.ownerOf(
            editionID3.toString()
        );
    ...
    it("test mint and burn successfully for tier 3", async function () {
        ...
        const newEditionID = result.receipt.rawLogs[logLength - 1].topics[3];
        const ownerNewEdition =
            await this.exhibitionTier3.ownerOf(newEditionID);
        ...
        const owner4New = await this.exhibitionV2.ownerOf(
            editionID4.toString()
        );
        const owner5New = await this.exhibitionV2.ownerOf(
            editionID5.toString()
        );
        const owner6New = await this.exhibitionV2.ownerOf(
            editionID6.toString()
        );
        const owner7New = await this.exhibitionV2.ownerOf(
            editionID7.toString()
        );
        const owner8New = await this.exhibitionV2.ownerOf(
            editionID8.toString()
        );
        const owner9New = await this.exhibitionV2.ownerOf(
            editionID9.toString()
        );
    ...
    it("test mint and burn fail because number of edition is invalid", async function () {
        ...
        try {
            ...
            assert.include(error.message, "Invalid number of burning editions");
        }
    ...
    it("test mint and burn fail because using same token input", async function () {
        ...
        try {
            ...
            assert.include(error.message, "Invalid burning editions");
        }
