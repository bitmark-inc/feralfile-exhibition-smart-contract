...
contract("FeralfileExhibitionV3_0", async (accounts) => {
    ...
    it("test batch transfer artworks successfully in 1 transaction", async function () {
        ...
        let ownerOfEdition = await this.exhibition.ownerOf(
            editionID.toString()
        );
        assert.equal(
    ...
    it("fail to batch transfer out of recv window", async function () {
        ...
        try {
            ...
            assert.include(error.message, "the transfer request is expired");
        }
    });
    ...
    it("test burn edition successfully", async function () {
        ...
        let editionCountBeforeBurn =
            await this.exhibition.totalEditionOfArtwork(this.artworkID);
